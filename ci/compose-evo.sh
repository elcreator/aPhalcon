...
# Sibling plugins built from a mounted checkout, if any (PLUGIN_SIBLINGS in
# ci/plugin.env). Each goes next to this plugin under core/custom/packages
# and is required from there; the rest come from packagist.
local_siblings=''
for sibling in ${PLUGIN_SIBLINGS:-}; do
    IFS='=' read -r s_package s_slug s_path s_version <<< "$sibling"
    if [ -d "$s_path" ]; then
        step "Sibling $s_package -> core/custom/packages/$s_slug (from $s_path)"
        s_dir="$target/core/custom/packages/$s_slug"
        rm -rf "$s_dir"
        mkdir -p "$s_dir"
        tar -c -C "$s_path" \
            --exclude=./.git \
            ...
            --exclude=./dist \
            . | tar -x -C "$s_dir"
        local_siblings="$local_siblings $s_package=$s_slug=${s_version:-}"
    else
        echo "sibling $s_package: no checkout at $s_path, composer will resolve it"
    fi
done
...
PLUGIN_PACKAGE="$PLUGIN_PACKAGE" PLUGIN_SLUG="$PLUGIN_SLUG" PLUGIN_LOCAL_SIBLINGS="${local_siblings# }" \
    php "$here/lib/write-custom-composer.php" "$custom"
cat "$custom"
...
# The siblings taken from a checkout are named too: they are @dev path
# packages now, and a partial update that does not name them keeps whatever
# the lock file says instead.
update_packages=("$PLUGIN_PACKAGE")
for sibling in $local_siblings; do
    update_packages+=("${sibling%%=*}")
done
( cd "$target/core" && composer update "${composer_args[@]}" --with-dependencies "${update_packages[@]}" )

...
// Sibling plugins copied in from a checkout, as "package=slug=version" words;
// each is one more path repository and one more requirement. The version, when
// given, is what the checkout is presented as - a checkout on a branch is
// dev-<branch> to composer, which no caret constraint accepts.
$siblings = array_filter(explode(' ', (string) (getenv('PLUGIN_LOCAL_SIBLINGS') ?: '')));
...
foreach ($siblings as $sibling) {
    [$siblingPackage, $siblingSlug, $siblingVersion] = explode('=', $sibling, 3) + [1 => '', 2 => ''];
    if ($siblingPackage === '' || $siblingSlug === '') {
        continue;
    }
    $options = ['symlink' => false];
    if ($siblingVersion !== '') {
        $options['versions'] = [$siblingPackage => $siblingVersion];
    }
    $data['repositories'][] = [
        'type' => 'path',
        'url' => 'packages/' . $siblingSlug,
        'options' => $options,
    ];
    $data['require'][$siblingPackage] = $siblingVersion !== '' ? $siblingVersion : '@dev';
}
